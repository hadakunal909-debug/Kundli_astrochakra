...
export const generateKundaliSVGNorthStyled = (kundli: Kundli, size = 500): string => {
    ...
    // ── Rashi numbers
    // North Indian charts have FIXED house positions (house 1 = top-centre holds
    // the Lagna). Each cell is labelled with the sign (rashi) number occupying it,
    // counting forward from the ascendant's rashi — NOT the house number.
    // ascendant.rashi is 1 = Aries … 12 = Pisces.
    const ascRashi = kundli.ascendant?.rashi ?? 1;
    const houseNums = Object.entries(H_NUM).map(([h, [hx, hy]]) => {
        const rashiNum = ((ascRashi - 1) + (Number(h) - 1)) % 12 + 1;
        return `<text x="${sc(hx)}" y="${sc(hy)}" text-anchor="middle" class="rashi-num">${rashiNum}</text>`;
    }).join("\n  ");
